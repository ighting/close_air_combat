...
Property.__new__.__defaults__ = (None, None, float(
    "-inf"), float("+inf"), "RW", Box, True, None)
...
class JsbsimCatalog(Property, Enum):
    ...
    attitude_heading_true_rad = Property(
        "attitude/heading-true-rad", "rad", access="R")
    attitude_psi_deg = Property(
        "attitude/psi-deg", "heading [deg]", 0, 360, access="R")
    attitude_psi_rad = Property("attitude/psi-rad", "rad", access="R")
    aero_beta_deg = Property(
        "aero/beta-deg", "sideslip [deg]", -180, +180, access="R")
    position_lat_geod_deg = Property(
        "position/lat-geod-deg", "geocentric latitude [deg]", -90, 90, access="R")
    position_lat_geod_rad = Property(
        "position/lat-geod-rad", "rad", access="R")
    ...
    velocities_u_aero_fps = Property(
        "velocities/u-aero-fps", "fps", access="R")
    velocities_v_aero_fps = Property(
        "velocities/v-aero-fps", "fps", access="R")
    velocities_w_aero_fps = Property(
        "velocities/w-aero-fps", "fps", access="R")
    ...
    velocities_p_rad_sec = Property(
        "velocities/p-rad_sec", "roll rate [rad/s]", -2 * math.pi, 2 * math.pi, access="R")
    ...
    velocities_r_rad_sec = Property(
        "velocities/r-rad_sec", "yaw rate [rad/s]", -2 * math.pi, 2 * math.pi, access="R")
    ...
    def update_equal_mixture_pos(sim):
        JsbsimCatalog.update_equal_engine_props(
            sim, JsbsimCatalog.fcs_mixture_pos_norm)

    def update_equal_feather_pos(sim):
        JsbsimCatalog.update_equal_engine_props(
            sim, JsbsimCatalog.fcs_feather_pos_norm)

    def update_equal_advance_pos(sim):
        JsbsimCatalog.update_equal_engine_props(
            sim, JsbsimCatalog.fcs_advance_pos_norm)
    ...
    fcs_flap_pos_norm = Property(
        "fcs/flap-pos-norm", "flap position, normalised", 0, 1)
    ...
    def update_equal_mixture_cmd(sim):
        JsbsimCatalog.update_equal_engine_props(
            sim, JsbsimCatalog.fcs_mixture_cmd_norm)

    def update_equal_advance_cmd(sim):
        JsbsimCatalog.update_equal_engine_props(
            sim, JsbsimCatalog.fcs_advance_cmd_norm)

    def update_equal_feather_cmd(sim):
        JsbsimCatalog.update_equal_engine_props(
            sim, JsbsimCatalog.fcs_feather_cmd_norm)
    ...
    gear_gear_cmd_norm = Property(
        "gear/gear-cmd-norm", "all landing gear commanded position, normalised", 0.0, 1.0)
    ...
    ic_h_sl_ft = Property(
        "ic/h-sl-ft", "initial altitude MSL [ft]", position_h_sl_ft.min, position_h_sl_ft.max)
    ...
    simulation_do_simple_trim = Property(
        "simulation/do_simple_trim", "", access="W")
...
class ExtraCatalog(Property, Enum):
    ...
    def update_delta_altitude(sim):
        value = (sim.get_property_value(ExtraCatalog.target_altitude_ft) -
                 sim.get_property_value(JsbsimCatalog.position_h_sl_ft)) * 0.3048
    ...
    def update_delta_velocities(sim):
        value = (sim.get_property_value(ExtraCatalog.target_velocities_u_mps) -
                 sim.get_property_value(ExtraCatalog.velocities_u_mps))
    ...
    def update_property_incr(sim, discrete_prop, prop, incr_prop):
        ...
        if value == 0:
            ...
            if value == 1:
                sim.set_property_value(prop, sim.get_property_value(
                    prop) - sim.get_property_value(incr_prop))
            elif value == 2:
                sim.set_property_value(prop, sim.get_property_value(
                    prop) + sim.get_property_value(incr_prop))
    ...
    def update_detect_extreme_state(sim):
        ...
        """
        extreme_velocity = sim.get_property_value(
            JsbsimCatalog.velocities_eci_velocity_mag_fps) >= 1e10
        extreme_rotation = (
            norm(
        ...
        extreme_altitude = sim.get_property_value(
            JsbsimCatalog.position_h_sl_ft) >= 1e10
        extreme_acceleration = (
            max(
                [
                    abs(sim.get_property_value(
    ...
    incr_throttle = Property(
        "fcs/incr-throttle", "incrementation throttle", 0, 1)
    ...
    incr_elevator = Property(
        "fcs/incr-elevator", "incrementation elevator", 0, 1)
    ...
    heading_check_time = Property(
        "heading_check_time", "time to check whether current time reaches heading time", 0, 1000000)
...
class MixedCatalog(dict):
    ...
    def add_jsbsim_props(self, jsbsim_props):
        ...
        """
        jsbsim_props_list = jsbsim_props.split("\n")
        for jsbsim_prop in jsbsim_props_list:
            if jsbsim_prop.strip() == "":
                continue  # skip empty line
            [name_jsbsim, access] = jsbsim_prop.split(" ")
            # remove parenthesis from the flag
            ...
            if name not in self:
                ...
                try:
                    ...
                    self[name] = Property(
                        name_jsbsim=name_jsbsim, access=access)
