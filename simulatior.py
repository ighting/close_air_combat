...
class AircraftSimulator(BaseSimulator):
    ...
    def __init__(self,
        ...
        """构造函数。创建 JSBSim 实例，加载飞机并设置初始条件.

        参数：
            uid (str)：用于唯一标识的 5 位十六进制数字。默认 = `"A0100"`。
            颜色（TeamColors）：使用不同的颜色字符串代表不同的团队
            model (str)：要装载的飞机的名称。默认 = `"f16"`。
                模型路径：'./data/aircraft_name/aircraft_name.xml'
            init_state (dict)：字典将属性映射到其初始值。输入空字典以使用一组默认的初始道具。
            origin（元组）：全球战场的原点（经度、纬度、海拔）。默认 = `(120.0, 60.0, 0.0)`
            sim_freq (int)：JSBSim 积分频率。默认值 = `60`。
        """
    ...
    def reload(self, new_state: Union[dict, None] = None, new_origin: Union[tuple, None] = None):
        ...
        # load JSBSim FDM(加载jsbsim的路径)
        # self.jsbsim_exec = jsbsim.FGFDMExec(
        #     os.path.join(get_root_dir(), 'data'))
        root_dir = 'C:\\Users\\upuplions\\anaconda3\\envs\\base2\\Lib\\site-packages\\jsbsim'
        self.jsbsim_exec = jsbsim.FGFDMExec(root_dir)
        # os.path.join:目录拼接
        self.jsbsim_exec.set_debug_level(0)
        self.jsbsim_exec.load_model(self.model)
        # print(self.jsbsim_exec.query_property_catalog(" "))
    ...
    def clear_defalut_condition(self):
        default_condition = {
            ...
            # initial (true) heading [deg] (0, 360)
            Catalog.ic_psi_true_deg: 0.0,
            # body frame x-axis velocity [ft/s]  (-2200, 2200)
            Catalog.ic_u_fps: 800.0,
            # body frame y-axis velocity [ft/s]  (-2200, 2200)
            Catalog.ic_v_fps: 0.0,
            # body frame z-axis velocity [ft/s]  (-2200, 2200)
            Catalog.ic_w_fps: 0.0,
            # roll rate  [rad/s]  (-2 * pi, 2 * pi)
            Catalog.ic_p_rad_sec: 0.0,
            # pitch rate [rad/s]  (-2 * pi, 2 * pi)
            Catalog.ic_q_rad_sec: 0.0,
            # yaw rate   [rad/s]  (-2 * pi, 2 * pi)
    ...
    def run(self):
        """运行 JSBSim 模拟，直到代理交互并更新自定义属性。

         JSBSim 监视模拟并检测它是否认为应该
          结束，例如因为指定了模拟时间。返回 false
          如果满足 JSBSim 终止条件。

          返回：
              (bool)：如果 sim 满足 JSBSim 终止条件，则为 False，否则为 True。
         """
        if self.is_alive:
            if self.bloods <= 0:
                self.shotdown()
    ...
    def _update_properties(self):
        ...
        self._position[:] = LLA2NEU(
            *self._geodetic, self.lon0, self.lat0, self.alt0)
        # 更新姿势
...
class MissileSimulator(BaseSimulator):
    ...
    def S(self):
        ...
        S0 += np.linalg.norm([np.sin(self._dtheta),
                             np.sin(self._dphi)]) * self._Diameter * self._Length
        return S0
    ...
    def launch(self, parent: AircraftSimulator):
        ...
        # remove missile 1s after its destroying
    ...
    def _guidance(self):
        ...
        # 计算 beta 和 eps，但实际上不需要......
        # beta = np.arctan2(y_m -y_t, x_m -x_t) # 相对偏航
        # eps = np.arctan2(z_m -z_t, np.linalg.norm([x_m -x_t, y_m -y_t])) # 相对螺距
        dbeta = ((dy_t - dy_m) * (x_t - x_m) -
                 (dx_t - dx_m) * (y_t - y_m)) / Rxy**2
